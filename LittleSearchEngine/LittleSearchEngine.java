...
public class LittleSearchEngine {
	...
	throws FileNotFoundException {
		HashMap<String,Occurrence> hashM = new HashMap<>();
		...
		while (sc.hasNext()) {
			...
			if(oc == null) {
				hashM.put(keyword, new Occurrence(docFile, 1));
			} else {
				oc.frequency++;
	...
	public void mergeKeywords(HashMap<String,Occurrence> kws) {
		...
		Iterator<Map.Entry<String, Occurrence>> it = set.iterator();
		    ...
		    while(it.hasNext()) {
			    ...
			    String st = pair.getKey();
			    Occurrence occ = pair.getValue();
			
			ArrayList<Occurrence> occs = keywordsIndex.get(st);
			    if(occs == null) {
				    occs = new ArrayList<>();
				    occs.add(occ);
				    keywordsIndex.put(st, occs);
			} else {
				occs.add(occ);
	...
	public String getKeyword(String word) {
		...
		for(int i = word.length() - 1; i >= 0; i--) {
			if(!delims.contains(word.charAt(i))) { 
	...
	public ArrayList<Integer> insertLastOccurrence(ArrayList<Occurrence> occs) {
		...
		while(lo <= hi) {
			...
			if(occs.get(mid).frequency == occ.frequency)
				break;
			
			else if(occs.get(mid).frequency > occ.frequency)
				lo = mid + 1;
			else
				hi = mid - 1;
		}
